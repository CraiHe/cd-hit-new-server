...
if (defined $q->param('JOBID')){ # retreive a existing job
  $JOBID = $q->param('JOBID');
  print $q->redirect("result.cgi?JOBID=$JOBID");
}
...
EOD
  ...
  # my $cmd = `$qsub_exe 1>$JOBDIR/$JOBID.qsub.dump 2>$JOBDIR/$JOBID.qsub.err.dump`;
  my $cmd = `sh $sh_fname 1>$JOBDIR/$JOBID.qsub.dump 2>$JOBDIR/$JOBID.qsub.err.dump`;
